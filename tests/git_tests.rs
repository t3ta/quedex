...
fn test_commit_message_generation() {
    ...
    let msg =
        quedex::git::generate_commit_message("Implement authentication", "task-1", "implement");
...
mod git_integration_tests {
    ...
    fn test_git_manager_create_commit_no_changes() {
        ...
        let commit_hash = manager
            ...
            .expect("Should handle no changes");
    ...
    fn test_git_manager_squash_commits() {
        ...
        let squash_hash = manager_final
            .squash_commits(3, "Squashed commit")
            .expect("Failed to squash commits");
        assert!(
            !squash_hash.is_empty(),
            "Squash should return a commit hash"
        );

        // Verify there are now 2 commits total (initial + squashed)
        let commits = manager_final
            .list_commits(10)
